...
// =====================================================
        ...
        function processKerberosStats(module, html) {
            ...
            props.iconSizeCustom = iconPreset.size;
            props.iconBackgroundSize = iconPreset.background;

            // KRITISCH: Template-ID überall ersetzen BEVOR wir die Stats generieren
            console.log('🔧 Ersetze {{templateId}} mit:', module.id);
            ...
            for (let i = 1; i <= 4; i++) {
                ...
                if (number && text && icon) {
                    console.log(`📊 Generiere Stat ${i} mit Hover-Klasse`);

                    // KRITISCH: HOVER-KLASSE MUSS HINZUGEFÜGT WERDEN
                    statsBlocks += `
                        <div class="kerberos-stat-card" style="background: ${props.cardBackground}; border: 1px solid ${props.cardBorder}; border-radius: ${props.cardRadius}; padding: ${props.cardPadding}; text-align: center; box-shadow: ${props.cardShadow};">
                            <div style="display: flex; align-items: center; justify-content: center; width: ${props.iconBackgroundSize}; height: ${props.iconBackgroundSize}; background: ${props.iconBackgroundColor}; border-radius: ${props.iconBackgroundRadius}; margin: 0 auto ${props.iconSpacing};">
                                <div class="kerberos-stat-icon" style="font-family: 'Font Awesome 5 Pro'; font-size: ${props.iconSizeCustom}; color: ${iconColor};">${icon}</div>
                            </div>
                            <div style="font-size: ${props.numberSize}; font-weight: 700; color: ${numberColor}; margin-bottom: ${props.numberSpacing};">${number}</div>
                            <div style="font-size: ${props.textSize}; color: ${textColor}; line-height: 1.4;">${text}</div>
                        </div>
                    `;
                }
            }

            console.log('🔧 Ersetze {{statsBlocks}} mit generiertem HTML');
            html = html.replace('{{statsBlocks}}', statsBlocks);

            console.log('✅ Process-Funktion abgeschlossen');
            return html;
        }

        // Füge diese Funktion direkt NACH der existierenden processKerberosStats Funktion hinzu

        function processKerberosStatsWithHover(module, html) {
            console.log('📊 processKerberosStatsWithHover aufgerufen für:', module.id);
            const props = module.properties;
            
            // Icon-Größen-Presets anwenden (wie in der originalen Funktion)
            const iconSizes = {
                'small': { size: '2rem', background: '60px' },
                'medium': { size: '2.5rem', background: '70px' },
                'large': { size: '3rem', background: '80px' },
            ...
            // Template-ID und andere kritische Platzhalter ersetzen
            html = html.replace(/\{\{templateId\}\}/g, module.id);
            html = html.replace(/\{\{hoverTransform\}\}/g, props.hoverTransform || 'translateY(-4px)');
            html = html.replace(/\{\{hoverShadow\}\}/g, props.hoverShadow || '0 8px 24px rgba(6,58,168,0.15)');
            html = html.replace(/\{\{mobileMinWidth\}\}/g, props.mobileMinWidth || '200px');

            // KRITISCH: Erstelle Stats-Content (nicht statsBlocks!)
            let statsContent = '';
            ...
            for (let i = 1; i <= 4; i++) {
                ...
                const text = props[`stat${i}Text`];
                const icon = props[`stat${i}Icon`];
                const iconColor = props[`stat${i}IconColor`] || '#063AA8';
                const numberColor = props[`stat${i}NumberColor`] || '#063AA8';
                const textColor = props[`stat${i}TextColor`] || '#6c757d';

                if (number && text && icon) {
                    console.log(`📈 Generiere Stat ${i} für statsContent`);

                    // Icon Background und Größen
                    const iconBgColor = props[`stat${i}IconBgColor`] || props.iconBackgroundColor || 'linear-gradient(135deg, #063AA8, #009CE6)';
                    const iconSize = props.iconSizeCustom || '3rem';
                    const iconBgSize = props.iconBackgroundSize || '80px';
                    const iconBgRadius = props.iconBackgroundRadius || '50%';
                    ...
                    const cardHoverTransform = props.cardHoverTransform || 'translateY(-8px)';
                    const cardHoverShadow = props.cardHoverShadow || '0 8px 24px rgba(6,58,168,0.15)';

                    statsContent += `
                        <div class="kerberos-stat-card kerberos-stat-hover-${module.id}" style="
                            ...
                            <!-- Number -->
                            <div style="
                                font-size: ${props.numberSize || '3rem'}; 
                                font-weight: 700; 
                                color: ${numberColor}; 
                    ...
                    // CSS für Hover-Effekte mit korrektem Cursor-Verhalten
                    const hoverCSS = `
                    <style>
                    ...
                    /* Cursor nur für echte Links */
                    .kerberos-stat-hover-${module.id} a,
                    .kerberos-stat-hover-${module.id} button,
                    .kerberos-stat-hover-${module.id} [onclick] {
                        cursor: pointer !important;
                    }
                    </style>
                    `;

                    // CSS vor dem HTML einfügen
                    html = hoverCSS + html;
                }
            }

            // KRITISCH: Ersetze {{statsContent}} (nicht {{statsBlocks}})
            html = html.replace('{{statsContent}}', statsContent);
            
            console.log('✅ processKerberosStatsWithHover abgeschlossen - statsContent ersetzt');
            return html;
        }
        ...
        function processTypeMappingsFallback(html, properties) {
            console.log('🔧 processTypeMappingsFallback wird ausgeführt für', Object.keys(properties).filter(k => k.endsWith('Type')).length, 'Type-Properties');
            
            // FEHLERBEHANDLUNG: Leere Properties abfangen
            if (!properties || typeof properties !== 'object') {
                console.warn('⚠️ processTypeMappingsFallback: Ungültige Properties');
                return html;
            }

            // Padding-Type Mappings
            const paddingMap = {
                'small': '0.5rem 1rem',
            ...
            // Ersetze Type-Properties mit echten CSS-Werten (OHNE properties zu überschreiben)
            Object.keys(properties).forEach(key => {
                if (key.endsWith('Type')) {
                    const value = properties[key];
                    let mappedValue = value;
                    
                    if (key.includes('Padding') || key.includes('Spacing')) {
                        mappedValue = paddingMap[value] || value;
                    } else if (key.includes('Radius')) {
                        mappedValue = radiusMap[value] || value;  
                    } else if (key.includes('Shadow')) {
                        mappedValue = shadowMap[value] || value;
                    } else if (key.includes('Size')) {
                        mappedValue = sizeMap[value] || value;
                    } else if (key.includes('Width') || key.includes('Height')) {
                        mappedValue = dimensionMap[value] || value;
                    }
                    
                    // Ersetze im HTML OHNE die original properties zu ändern
                    const regex = new RegExp(`{{${key}}}`, 'g');
                    html = html.replace(regex, mappedValue);
                }
            });
            
            return html; // KORRIGIERT: "ret" -> "return html;"
        }
